...
const getCurrentYearTimesheetsForUser = (req, res) => {
  ...
  console.table({ nextYear });

  // Construct the SQL query to fetch timesheets from the current year to the next year for a specific user
  const query = `
      ...
      LEFT JOIN ts_timesheet_t ON ts_timesheet_t.work_date = dates.date AND ts_timesheet_t.person_id = $3
      WHERE EXTRACT(YEAR FROM dates.date) BETWEEN $1 AND $2
      ORDER BY dates.date, ts_timesheet_t.id;
  
  `;
  console.log(
    "cyt5   " +
      "SELECT * FROM ts_timesheet_t WHERE EXTRACT(YEAR FROM work_date) BETWEEN " +
      currentYear +
      " AND " +
      nextYear +
      " AND person_id = " +
  ...
  // Execute the query with currentYear, nextYear, and personID as parameters
  pool.query(query, [currentYear, nextYear, personID], (error, result) => {
    if (error) {
      console.error("Error querying timesheets:", error);
      res.status(500).json({ error: "Error querying timesheets" });
      return;
...
const createTimesheet = async (req, res) => {
  ...
  pool.query(
    "DELETE FROM ts_timesheet_t WHERE work_date = $1 and person_id = $2 ",
    [work_date, person_id],
    (error, results) => {
      if (error) {
        throw error;
      ...
      status =  draft == 'true' ? 'draft' : status
...
const getUserByEmail = (req, res) => {
  console.log("qw1   ", req.params.email);
  console.log("qw2 SELECT * FROM users WHERE email = '" + req.params.email + "';");
  
  pool.query("SELECT * FROM users WHERE email = '" + req.params.email + "';",
    (error, result) => {
      // console.log("qw2    ");
      // if (error) {
      ...
      if (error) {
        console.log("qw3      db error", error);
        throw error;
      } else {
        console.log("qw4   ", result)
